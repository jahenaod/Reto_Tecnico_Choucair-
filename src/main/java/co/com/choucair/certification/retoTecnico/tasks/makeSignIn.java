...
public class makeSignIn implements Task {
    ...
    public <T extends Actor> void performAs(T actor){
        actor.attemptsTo(Click.on(AutomationPage.BUTTON_JOIN_TODAY),
                ...
                WaitUntil.the(AutomationPage.BUTTON_LOCATION, isVisible()).forNoMoreThan(15).seconds(),
                Click.on(AutomationPage.BUTTON_LOCATION),
                WaitUntil.the(AutomationPage.BUTTON_DEVICE, isVisible()).forNoMoreThan(15).seconds(),
                Click.on(AutomationPage.BUTTON_DEVICE),
                WaitUntil.the(AutomationPage.BUTTON_NEXT_STEP, isVisible()).forNoMoreThan(60).seconds(),
                Click.on(AutomationPage.BUTTON_NEXT_STEP),
                WaitUntil.the(AutomationPage.PASSWORD, isVisible()).forNoMoreThan(15).seconds(),
